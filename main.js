...
/* ── 텍스트 모드 랜덤 선택 ── */
var 랜덤이유목록 = [
  '그냥 느낌이 왔어요. 직감은 배신하지 않거든요.',
  '오늘 날씨랑 잘 어울리는 것 같아서요.',
  '우주의 기운이 이쪽을 가리키고 있었어요.',
  '고민하는 시간이 아깝잖아요. 그냥 이거 하세요!',
  '다른 걸 선택했으면 평생 후회했을 거예요.',
  '상상해보세요. 이거 선택하면 기분이 좋아질 것 같지 않나요?',
  '반대쪽도 나쁘진 않지만, 오늘만큼은 이게 정답이에요.',
  '제 촉이 99% 맞거든요. 나머지 1%는 무시하세요.',
  '고민이 길어지면 결국 이걸 선택하게 되어 있어요.',
  '이거 선택하면 오늘 하루 운이 트일 것 같은 느낌적인 느낌!',
  '통계적으로 이쪽이 더 행복지수가 높아요. (근거는 없음)',
  '이미 마음속으로 이미 이걸 원하고 있었잖아요?'
];

function 텍스트랜덤선택() {
  return {
    winner:   Math.random() < 0.5 ? 'A' : 'B',
    reasoning: 랜덤이유목록[Math.floor(Math.random() * 랜덤이유목록.length)]
  };
}
